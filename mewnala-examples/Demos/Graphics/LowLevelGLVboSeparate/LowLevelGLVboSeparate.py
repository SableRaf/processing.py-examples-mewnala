...
def allocate_direct_float_buffer(n):
    return (ByteBuffer.allocate_direct(n * Float.SIZE / 8).
            order(ByteOrder.native_order()).as_float_buffer())
...
def draw():
    with g.begin_p_g_l() as pgl:
	    ...
	    # fill VBO with data
	    pgl.buffer_data(
	        PGL.ARRAY_BUFFER, Float.BYTES * len(vertices), vertex_buffer, PGL.DYNAMIC_DRAW)
	    # associate currently bound VBO with "vertex" shader attribute
	    pgl.vertex_attrib_pointer(
	        vert_loc, VERT_CMP_COUNT, PGL.FLOAT, False, vertex_stride, vertex_offset)
	    ...
	    # fill VBO with data
	    pgl.buffer_data(
	        PGL.ARRAY_BUFFER, Float.BYTES * len(colors), color_buffer, PGL.DYNAMIC_DRAW)
	    # associate currently bound VBO with "color" shader attribute
	    pgl.vertex_attrib_pointer(
	        color_loc, CLR_CMP_COUNT, PGL.FLOAT, False, color_stride, color_offset)
