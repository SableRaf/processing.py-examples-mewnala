...
def mouse_pressed():
    global current_gesture_i_d
    current_gesture_i_d = (current_gesture_i_d + 1) % n_gestures
    gesture = gesture_array[current_gesture_i_d]
    gesture.clear()
    gesture.clear_polys()
...
def key_pressed():
    if key in ('+', '='):
        if current_gesture_i_d >= 0:
            th = gesture_array[current_gesture_i_d].thickness
            gesture_array[current_gesture_i_d].thickness = min(96, th + 1)
            gesture_array[current_gesture_i_d].compile()
    elif key == '-':
        if current_gesture_i_d >= 0:
            th = gesture_array[current_gesture_i_d].thickness
            gesture_array[current_gesture_i_d].thickness = max(2, th - 1)
            gesture_array[current_gesture_i_d].compile()
    elif key == ' ':
        clear_gestures()
...
def update_geometry():
    for i in range(n_gestures):
        ...
        if not gesture.exists:
            continue
        if (i != current_gesture_i_d or not mouse_is_pressed):
