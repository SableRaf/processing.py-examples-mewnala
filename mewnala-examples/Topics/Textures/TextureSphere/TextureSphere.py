...
def initialize_sphere(res):
    ...
    for i in range(SinCosLength):
        sin_l_u_t.append(sin(radians(i * SinCosPrecision)))
        cos_l_u_t.append(cos(radians(i * SinCosPrecision)))
    delta = SinCosLength / res

    # Calc unit circle in XZ plane.
    for i in range(res):
        cx.append(-cos_l_u_t[int((i * delta) % SinCosLength)])
        cz.append(sin_l_u_t[int((i * delta) % SinCosLength)])
    ...
    for i in range(1, res):
        curradius = sin_l_u_t[int(angle % SinCosLength)]
        curr_y = -cos_l_u_t[int(angle % SinCosLength)]
