...
class Icosahedron(Shape3D):
    ...
    def __init__(self, radius=150):
        ...
        angle = 0
        for i in range(5):
            self.top_pent[i] = PVector(cos(angle) * radius,
                                      sin(angle) * radius,
                                      self.tri_height / 2.0)
            angle += radians(72)
        self.top_point = PVector(0, 0, self.tri_height / 2.0 + a)
        angle = 72.0 / 2.0
        for i in range(5):
            self.bottom_pent[i] = PVector(cos(angle) * radius,
                                         sin(angle) * radius,
                                         -self.tri_height / 2.0)
            angle += radians(72)
        self.bottom_point = PVector(0, 0, -(self.tri_height / 2.0 + a))
